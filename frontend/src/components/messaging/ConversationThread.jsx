...
// Fil de conversation pour un conversationId donné — poll léger (5s) plutôt
// que du temps réel SSE : la messagerie est scopée à un contexte métier
// (commande/livraison/transaction B2B), pas un usage type messagerie
// instantanée à fort volume, un polling court suffit sans complexité
// supplémentaire côté transport.
export default function ConversationThread({ conversationId }) {
  ...
  const send = async (e) => {
    ...
    setError('')
    try {
      const msg = await api.post(`/conversations/${conversationId}/messages`, { content })
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.length === 0 ? (
          <p className="text-center font-dm text-sm text-charcoal/40 mt-8">Aucun message. Dites bonjour !</p>
        ) : messages.map((m) => {
